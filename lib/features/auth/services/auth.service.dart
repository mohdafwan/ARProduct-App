...
class AuthServiceSignUp {
  ...
  void signUpUser({
    ...
    try {
      ...
      http.Response res = await http.post(
        Uri.parse('http://192.168.0.107:3000/api/register'),
        body: user.toJson(),
        headers: {
  ...
  void signInUser({
    ...
    try {
      http.Response res = await http.post(
        Uri.parse('http://192.168.0.107:3000/api/login'),
        body: jsonEncode({
          'email': email,
          'password': password,
        }),
  ...
  void getUserData(
    ...
    try {
      ...
      var tokenRes = await http.post(
        Uri.parse('http://192.168.0.107:3000/tokenIsValid'),
      ...
      if (response == true) {
        http.Response userRes = await http.get(
          Uri.parse('http://192.168.0.107:3000/'),
          headers: <String, String>{
            'Content-Type': 'application/json; charset=UTF-8',
            'x-auth-token': token
          },
